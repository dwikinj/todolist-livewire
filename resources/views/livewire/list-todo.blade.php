        ...
        @foreach ($todos as $todo)
            <div class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
                <div class="flex flex-col w-full">
                        ...
                        <div class="flex justify-between space-x-2">
                            ...
                            <div class="flex items-center space-x-2">
                                <button wire:click='updateTodo()'
                                    class="px-4 py-2 bg-teal-500 text-white text-sm font-semibold rounded hover:bg-teal-600">Update</button>
                                <button wire:click='cancelEditing()'
                                    class="px-4 py-2 bg-red-500 text-white text-sm font-semibold rounded hover:bg-red-600">Cancel</button>
                            </div>
                        </div>
                    @else
                        <div class="flex justify-between space-x-2">
                            ...
                            <div class="flex items-center space-x-2">
                                {{-- edit button --}}
                                <button wire:click='startEditing({{ $todo->id }})'
                    ...
                    @endif
                    <span class="text-xs text-gray-500 mt-1"> {{ $todo->created_at->format('d/m/Y') }}</span>
                </div>
            </div>
        @endforeach
